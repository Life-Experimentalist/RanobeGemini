...
// Cache for the content area to prevent redundant lookups
let cachedContentArea = null;
let lastVersionCheck = null; // Store whether the last check was mobile or desktop
...
class FanfictionHandler extends BaseWebsiteHandler {
	 ...
	 */
	findContentArea() {
		// Use cached version if available and we're on the same version (mobile/desktop)
		const isMobile = this.isMobileVersion();

		if (cachedContentArea && lastVersionCheck === isMobile) {
		...
		if (isMobile) {
			...
			if (contentElement) {
				...
				cachedContentArea = contentElement;
				lastVersionCheck = isMobile;
