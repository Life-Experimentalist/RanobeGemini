...
- After each validated prompt cycle, update this roadmap's rolling tracker and any affected concise status docs so the workflow record stays current.
- When a cycle changes module topology, consult or refresh `graphify-out/GRAPH_REPORT.md` before closing the cycle so the structural audit matches the codebase.

## Prompt Selection Workflow & AI Token Management

### Are we wasting tokens using Graphify?
**No, Graphify saves tokens.**
- **Zero API Cost ($0.00, 0 tokens):** Running `npm run docs:graphify` (which triggers `python -m graphify update .`) strictly relies on local AST (Abstract Syntax Tree) parsing. It scans structure without sending code to an LLM.
- **Massive Context Savings:** Instead of the AI needing to parse and `read_file` on 30 different JS files to understand the project architecture, it only needs to read `graphify-out/GRAPH_REPORT.md` (a plain-text map). This keeps the prompt context small and efficient.

### Which Prompt to Use?

1. **The Bulldozer (`@workspace /prompt get-to-work`)**
   - **When:** Use for continuous, highly autonomous, multi-phase loops.
   - **Action:** The AI reads this roadmap, picks the next incomplete task, codes it, validates (lint/build), runs `docs:graphify` to sync the map, marks the task as `Done` in this file, and immediately starts the next task without waiting for human approval.
2. **The Next Step (`@workspace /prompt roadmap-continue-autonomous`)**
   - **When:** Use to resume a paused autonomous task or execute a single, tight phase boundary.
   - **Action:** The AI completes exactly what was previously active, validates, graphs, marks as `Done`, and stops for review.
3. **The Sniper (`@workspace /prompt roadmap-implementation <instructions>`)**
   - **When:** Use for highly specific or manual tasks that aren't mapped clearly to a large lifecycle phase.
4. **Release Cuts (`@workspace /prompt release-notes`)**
   - **When:** Use when turning completed work into a release package.
...
| 1-U5b Orchestrator closure sweep continuation (L1)                      | 2-3              | 4              | +1.5 vs midpoint | Extracted enhance prechecks/lifecycle, removed duplicated `handleReenhanceChunk` inline fallback, and validated lazy-loaded runtime delegation                | partially-valid   | Keep closure slices small; complete parity smoke checklist and then close Phase 1                    | 2026-04-19   |
| 1-U5c Targeted parity smoke checks (L1)                                 | 1-2              | 1              | -0.5 vs midpoint | Revalidated lint/build after delegation-only runtime paths for cached, chunked, and re-enhance orchestration                                                  | valid             | Mark Phase 1 closure candidate and move next priority to Phase 2-U1                                  | 2026-04-19   |
| 1-U5d Cached chunk restore runtime extraction (L0-L1)                   | 1-2              | 1              | -0.5 vs midpoint | Moved `restoreChunkedContentFromCache` orchestration into `chunk-batch` runtime helper and kept delegator-only flow in `content.js`                           | valid             | Continue reducing remaining large content orchestrators before Phase 1 closeout                      | 2026-04-19   |
| 1-U5e Status toast runtime delegation (L0-L1)                           | 1-2              | 1              | -0.5 vs midpoint | Delegated `showStatusMessage` rendering/logging path to `notification-runtime` and preserved fallback behavior in `content.js`                                | valid             | Continue shrinking remaining notification/banner orchestration in Phase 1                            | 2026-04-19   |
| 1-U5f Timed banner runtime delegation + exposure fix (L0-L1)            | 1-2              | 1              | -0.5 vs midpoint | Exposed notification runtime helpers via `createNotificationRuntime` and delegated `showTimedBanner` to module runtime while preserving fallback              | valid             | Continue next notification-orchestration split while preserving parity safeguards                    | 2026-04-19   |
| 1-U5g Timed banner fallback collapse + field updater extraction (L0-L1) | 1-2              | 1              | -0.5 vs midpoint | Reduced `showTimedBanner` fallback to status-path safety fallback and extracted `updateBannerField` runtime helper to `notification-runtime`                  | valid             | Continue reducing remaining notification wrappers and then re-evaluate Phase 1 closure               | 2026-04-19   |
| 1-U5h Status fallback collapse (L0-L1)                                  | 1-2              | 1              | -0.5 vs midpoint | Reduced `showStatusMessage` inline fallback to compact safety path while keeping runtime-backed notification/logging flow unchanged                           | valid             | Re-run parity smoke checks, then decide Phase 1 closeout vs final split                              | 2026-04-19   |
| 1-U5i Popup message handler parity restore + validation (L1)            | 1-2              | 1              | -0.5 vs midpoint | Restored `getSiteHandlerInfo` and `testExtraction` message actions removed during refactor and revalidated lint/build parity                                  | valid             | Continue Phase 1 orchestrator thinning with explicit popup/background contract checks                | 2026-04-19   |
| 1-U5j Popup/library runtime extraction (L0-L1)                          | 1-2              | 1              | -0.5 vs midpoint | Delegated `getNovelInfo` and `addToLibrary` message orchestration to `popup-library-runtime` while preserving in-file fallback parity                         | valid             | Continue trimming remaining popup/library orchestrators and close Phase 1 with contract smoke checks | 2026-04-19   |
| 1-U5k Reading-status popup runtime delegation (L0-L1)                   | 1-2              | 1              | -0.5 vs midpoint | Delegated `updateNovelReadingStatus` message handling through `popup-library-runtime` and retained fallback path in `content.js`                              | valid             | Re-run popup/library message contract checks and then evaluate Phase 1 closeout                      | 2026-04-19   |
| 1-U5l Popup contract helper delegation (L0-L1)                          | 1-2              | 1              | -0.5 vs midpoint | Delegated `getSiteHandlerInfo` and `testExtraction` message paths through popup runtime helpers with listener fallback parity retained                        | valid             | Run contract-focused parity validation and decide final Phase 1 closure slice                        | 2026-04-19   |
| 1-U5m Processing-cancelled listener delegation (L0-L1)                  | 1-2              | 1              | -0.5 vs midpoint | Delegated `processingCancelled` listener UI/reset handling to `enhancement-cancel` runtime helper with fallback path retained                                 | valid             | Re-run cancellation + popup contract parity checks and evaluate Phase 1 closeout                     | 2026-04-19   |
| 1-U5n API-key-missing listener delegation (L0-L1)                       | 1-2              | 1              | -0.5 vs midpoint | Delegated `apiKeyMissing` listener UI/reset handling to `enhancement-cancel` runtime helper while preserving popup-open and fallback copy parity              | valid             | Re-run listener contract parity checks and evaluate whether one final closure slice is needed        | 2026-04-19   |
| 1-U5o Listener async response wrapper consolidation (L0-L1)             | 1-2              | 1              | -0.5 vs midpoint | Consolidated repeated promise-based `sendResponse` wiring into one helper for popup/process/summarize listener branches without payload changes               | valid             | Run final Phase 1 parity sweep and determine closeout eligibility                                    | 2026-04-19   |
| 1-U5p Popup/content listener parity sweep + closeout candidate (L1)     | 1-2              | 1              | -0.5 vs midpoint | Verified popup-dispatched tab message actions map cleanly to content listener handlers after modularization (`ping`, handler info, library actions, settings) | valid             | Treat Phase 1 as closure-candidate and continue Phase 2 backlog items with periodic parity checks    | 2026-04-19   |
...
| 10-U1 Novel context extraction to novel-context.js (L0-L1)              | 2-3              | 2              | -0.5 vs midpoint | 12 functions extracted (863 lines removed); zero lint errors; delegator wired; module loader injected into initialize()                                      | valid             | Continue 10-U2 UI DOM generation extractions next                                                    | 2026-04-20   |
