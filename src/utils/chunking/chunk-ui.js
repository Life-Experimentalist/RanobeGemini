...
export function createChunkBanner(
	...
	if (wordCounts && (wordCounts.original || wordCounts.enhanced)) {
		...
		if (absDiffPercent > threshold) {
			...
			thresholdWarning = `
				<div class="rg-threshold-warning" style="
					display: flex;
					align-items: center;
					gap: 8px;
				...
				">
					...
					<button id="${showEnhancedBtnId}" class="rg-show-enhanced-btn" style="
						padding: 3px 10px;
						background: #374151;
						color: #e5e7eb;
	...
	// Wire up Show Enhanced button in threshold warning
	banner.addEventListener("click", (e) => {
		const btn = e.target.closest(".rg-show-enhanced-btn");
		if (!btn) return;
		const wrapper = banner.closest(".gemini-chunk-wrapper");
		if (wrapper) {
			const chunkContent = wrapper.querySelector(".gemini-chunk-content");
			if (chunkContent) chunkContent.style.display = "";
		}
		btn.closest(".rg-threshold-warning")?.remove();
	});
