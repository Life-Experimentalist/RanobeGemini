...
export class NovelLibrary {
	...
	async prepareUrlsForImport(urls = []) {
		const library = await this.getLibrary();
		const novels = library.novels || {};
		const existingNovelIds = new Set(Object.keys(novels));
		...
		for (const rawUrl of urls) {
			...
			if (!url) {
				...
				result.unsupportedItems.push({ url: rawUrl || "" });
				continue;
			}
			...
			if (!identity) {
				result.unsupported += 1;
				result.unsupportedItems.push({ url });
				continue;
			}

			if (existingNovelIds.has(identity.novelId)) {
				result.skippedExisting += 1;
				const existing = novels[identity.novelId] || {};
				result.existingItems.push({
					url,
			...
			if (seenNovelIds.has(identity.novelId)) {
				...
				result.duplicateItems.push({ url, novelId: identity.novelId });
				continue;
			}
