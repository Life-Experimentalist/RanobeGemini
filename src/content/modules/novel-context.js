 ...
 *
 * Originally lifted out of content.js in one pass. Nothing regenerates it, so
 * it is maintained here directly — `ctx` is the contract with content.js and
 * every key destructured below must be one this module actually uses.
 */
...
export function initNovelContextModule(ctx) {
	...
	async function autoUpdateNovelOnVisit() {
		console.log("[RG-Library-Debug] autoUpdateNovelOnVisit called", {
			url: window.location.href,
		});
		...
		if (!novelLibrary) {
			console.log(
				"[RG-Library-Debug] ABORT: novelLibrary not available after loadNovelLibrary()",
			);
		...
		try {
			...
			if (
				...
				console.log(
					"[RG-Library-Debug] dedicated_page chapter: novelPageUrl =",
					novelPageUrl,
				);
				if (novelPageUrl) {
					...
					console.log(
						"[RG-Library-Debug] dedicated_page chapter: novelId =",
						novelId,
						"existingNovel =",
						existingNovel?.title ?? null,
					);
			...
			if (typeof currentHandler.extractNovelMetadata !== "function") {
				console.log(
					"[RG-Library-Debug] ABORT: handler has no extractNovelMetadata()",
				);
			...
			if (!metadata || !metadata.title) {
				console.log(
					"[RG-Library-Debug] ABORT: metadata.title is empty/null — cannot auto-add",
				);
			...
			const novelIdSource = novelId
				? "handler.generateNovelId()"
				: "url-hash fallback";
			if (!novelId) {
				const shelfId =
			...
			console.log(
				"[RG-Library-Debug] library check: totalNovels =",
				existingNovels.length,
				"existingNovel =",
				existingNovel?.title ?? "(not found)",
			);
			...
			if (existingNovel) {
				...
				if (existingNovel.pendingRefresh === true) {
					debugLog(
						`📚 pendingRefresh flag set — forcing full metadata update for ${existingNovel.title}`,
					);
				...
				if (autoAddEnabled && metadata.title && !blocklisted) {
					// Build novel data
					...
					console.log("[RG-Library-Debug] calling addOrUpdateNovel", {
						id: novelData.id,
					...
					console.log(
						"[RG-Library-Debug] addOrUpdateNovel SUCCESS — novel auto-added:",
						novelData.title,
					);
			...
			console.log(
				"[RG-Library-Debug] ERROR in autoUpdateNovelOnVisit:",
				error?.message,
				error,
			);
