 ...
 *
 * @param {Object<string, Set>} categoryLookup - initial category → Set map (e.g. { genres: new Set(), tags: new Set() })
 ...
 * @param {Object<string, number>} buckets - status → count
 * @param {number} total
 */
...
export function syncMultiSelectUI(containerId, selectedValues, onChange) {
	...
	boxes.forEach((box) => {
		...
		// Avoid duplicate listeners — clone-in-place pattern
