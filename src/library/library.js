...
let allNovels = [];
let siteSettings = {};
const _recentRandomPicks = new Set();
let librarySettings = {
...
const elements = {
	...
	queueActiveBadge: document.getElementById("queue-active-badge"),
...
function initQueueStatusWidget() {
	...
	const badge = elements.queueActiveBadge;
	if (!btn || !badge) return;
...
function setupEventListeners() {
	...
	if (elements.libraryRandomBtn) {
		elements.libraryRandomBtn.addEventListener("click", () => {
			...
			let candidates = pool.filter((n) => !_recentRandomPicks.has(n.id));
			...
			if (_recentRandomPicks.size > Math.ceil(pool.length / 2)) {
				...
				_recentRandomPicks.delete(oldest);
			}
...
function populateContinueReadingHero(novels) {
	...
	if (elements.heroContinueBtn) {
		elements.heroContinueBtn.href =
			hero.lastReadChapterUrl ||
			hero.currentChapterUrl ||
			hero.lastReadUrl ||
			hero.sourceUrl;
